...
int unreliable_getattr(const char *path, struct stat *buf)
{
    ...
    printf("NOTICE:: performing getattr of %s\n", path);
    int res = wrap_getattr(path, buf);
    return res;
}
...
int unreliable_rename(const char *oldpath, const char *newpath)
{
    ...
    ret = wrap_rename(oldpath, newpath);
    if (ret == -1) {
        return -errno;
    }
...
int unreliable_open(const char *path, struct fuse_file_info *fi)
{
    ...
    printf("NOTICE:: performing open of %s\n", path);

    ret = wrap_open(path, fi);
                    ...
                    struct fuse_file_info *fi)
{
    ...
    ret = wrap_read(fd, buf, size, offset);
    if (ret == -1) {
        ret = -errno;
    }
                       ...
                       off_t offset, struct fuse_file_info *fi)
{
    ...
    int res;
    res = wrap_readdir(path, buf, filler);
    if (res < 0) {
        return res;
    }
