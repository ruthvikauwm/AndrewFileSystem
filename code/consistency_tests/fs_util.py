...
def start_another_client(host: str, test_case: int, client_id: str,
    ...
    script_name = f'~/wiscAFS/consistency_tests/test{test_case}_client{client_id.upper()}.py'
    ssh_cmd = f'source ~/wiscAFS/consistency_tests/739p1.env && python3 {script_name}'
...
def path_exists(path_name: str) -> bool:
    print(f'{path_name} path exists?',os.path.exists(path_name))
